document.addEventListener('DOMContentLoaded', () => {
    ...
    // Utility: Parse numbers safely. If the string is empty (unreleased event), return NaN
    ...
    const calculateBias = (data) => {
        ...
        data.forEach(row => {
            ...
            // Skip calculation entirely if data hasn't been released yet
    ...
    const renderTable = (data) => {
        ...
        const CATEGORIES = {
            ...
            "Economic Growth & Activity": ["GDP", "PMI", "Retail", "Confidence", "Building", "Manufacturing", "Services", "Philly Fed"],
        ...
        const getCategory = (indicatorName) => {
            ...
            // Force Unemployment check first
        ...
        Object.keys(groupedData).forEach(category => {
            ...
            rowsInCategory.forEach(row => {
                ...
                let effectLabel = "Pending"; // Default to pending for unreleased data

                // Math only fires if the actual event has been released
                ...
                } else if (!isNaN(actualNum) && isNaN(forecastNum)) {
                    effectLabel = "Neutral"; // Catch edge-cases where data drops without forecast consensus
                }

                // Prevent clumsy table layouts by replacing blank spaces with nice dashes
                const displayPrev = row.Previous && row.Previous.trim() !== '' ? row.Previous : '-';
                const displayFore = row.Forecast && row.Forecast.trim() !== '' ? row.Forecast : '-';
                const displayAct  = row.Actual && row.Actual.trim() !== '' ? row.Actual : '-';
                ...
                tr.innerHTML = `
                    ...
                    <td style="color: var(--text-muted)">${displayPrev}</td>
                    <td>${displayFore}</td>
                    <td class="${resultClass}" style="font-weight: 800;">${displayAct}</td>
